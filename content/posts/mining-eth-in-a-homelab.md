...
With the guest up, the rest is mundane. Install the miner and join a pool - a pool being the arrangement where many small miners submit work together and split the proceeds, because a lone consumer card has no realistic chance of finding a block on its own. Pools differ in the balance they owe you before they will pay it out, and I picked ethermine because its 0.1 ETH floor was the lowest I found. With a small hashrate that threshold is the thing that decides whether payouts are a regular event or a theoretical one. I did not keep records of which it turned out to be.
...
Heat was the real problem. GPUs crammed into a workstation chassis cook. I bought a 4U case - four rack units, about 17.8 cm of external height, which is the smallest rack format that takes a full-height card standing up with airflow around it rather than pressed against its neighbour - and dedicated it to this. Thermally that configuration ran about three months without drama. That is a claim about the hardware, not about the Windows guest running on it; those two things were stable and unstable respectively, at the same time.
...
- Set `hostAliases` for the pool's domains, which pins the names to addresses in the pod's own `/etc/hosts` so the lookups never reach the cluster resolver.

  This one needs explaining, because out of context it reads badly. I had a firewall rule on my own network that flagged DNS lookups for known mining pools - I had written it myself, for exactly the reason you would - and my own miner kept tripping it. `hostAliases` was how I got out of the way of my own alerting.

  I have left it in rather than quietly dropping it, because it is the most useful thing in this section. It took about a minute to defeat a detection I had built deliberately, on infrastructure I controlled, with a stock Kubernetes field and no privileges. Anyone who can set a pod spec can do it. That is the argument for why DNS-based and signature-based mining detection is close to worthless, and it is why the [companion post](/posts/the-economics-that-make-cryptojacking-work/) tells defenders to watch utilisation and billing instead of names and binaries. I did not arrive at that conclusion from theory.

  It is not free, either: pinned addresses go stale when the pool rotates its DNS, and the pod carries on mining to nowhere until somebody notices.
